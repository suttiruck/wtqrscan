...
import 'package:wtqrscan/class/CustomGradientAppBar.dart';
          ...
          if (data != null) {
            setState(() {
              ...
              if (data1 != null && data1['data'] != null) {
                Share.LocationIN_ID = data1['data']['location_id'] ?? 0;
                Share.LocationIN_Name =
                    data1['data']['location_description'] ?? 'N/A';
                Share.LocationIN_Keep = data1['data']['remark'] ?? 'N/A';
              } else {
                // ตั้งค่าหากไม่พบข้อมูลที่ต้องการ
                Share.LocationIN = '';
                Share.LocationIN_ID = 0;
                Share.LocationIN_Name = 'N/A';
                Share.LocationIN_Keep = 'N/A';
                if (resultData != '-1') {
                  alert(context,
                      title: 'Massage Box',
  ...
  Widget location(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: (Share.LocationIN?.isEmpty ?? true)
            ? Colors.red[100]
            : Colors.green[100], // เปลี่ยนสีตามสถานะของ LocationIN
        border: Border.all(
          color:
              (Share.LocationIN?.isEmpty ?? true) ? Colors.red : Colors.green,
          width: 2,
        ),
        borderRadius: BorderRadius.circular(8),
      ),
      child: Text(
        (Share.LocationIN?.isEmpty ?? true)
            ? 'Please Scan Location'
            : "Location : ${Share.LocationIN_Name} Remark : ${Share.LocationIN_Keep}",
        style: TextStyle(
          fontSize: fontSize,
        ...
        child: Center(
          child: Column(
            children: [
              ...
              const SizedBox(height: 0),
              if (Share.LocationIN != null && Share.LocationIN!.isNotEmpty) ...[
